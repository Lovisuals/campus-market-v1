...
export default function ProductCard({ item, viewMode, isAdmin, userLoc, campuses, handlers }) {
    ...
    // --- RENDER: MARKET ITEM (Selling) ---
    if (viewMode === 'market') {
        return (
            <div className={`relative bg-white dark:bg-gray-900 rounded-2xl overflow-hidden shadow-sm border border-gray-100 dark:border-gray-800 ${item.is_admin_post ? 'ring-2 ring-yellow-400' : ''}`}>
                ...
                <div className="aspect-square bg-gray-100 relative overflow-hidden group">
                    ...
                    <div className="flex overflow-x-auto snap-x snap-mandatory scrollbar-hide w-full h-full">
                         {item.images && item.images.length > 0 ? item.images.map((img, idx) => (
                            <img 
                                key={idx} 
                                src={img} 
                                onClick={() => handlers.setFullscreenImg(img)} 
                    ...
                    {/* Dots Indicator */}
                    {item.images?.length > 1 && <div className="absolute bottom-2 left-0 right-0 flex justify-center gap-1">{item.images.map((_, i) => <div key={i} className="w-1 h-1 rounded-full bg-white/80 shadow-sm"></div>)}</div>}

                    {/* Badges */}
                    {isSold && <div className="absolute inset-0 bg-black/60 flex items-center justify-center"><span className="text-white font-black border-2 px-1 py-0.5 text-[8px] -rotate-12">SOLD</span></div>}
                    {item.is_verified && <div className="absolute top-1 right-1 bg-yellow-400 text-black text-[8px] font-black px-1.5 py-0.5 rounded-full shadow-md flex items-center gap-1"><span>🛡️</span></div>}
                </div>
    ...
    // --- RENDER: REQUEST ITEM (Buying) ---
    return (
        <div className="relative bg-white dark:bg-gray-900 rounded-2xl overflow-hidden shadow-sm border-l-4 border-l-[#8E44AD]">
            <div className="p-3 flex flex-col h-full justify-between">
                <div>
                    ...
                    <h3 className="text-[10px] font-bold leading-tight mb-2 line-clamp-2">{item.title}</h3>
                    
                    {/* Tiny Reference Image */}
                    {item.image_url && (
                        <img 
                            src={item.image_url} 
                            onClick={() => handlers.setFullscreenImg(item.image_url)} 
                            className="w-8 h-8 rounded-md object-cover mb-2 border border-gray-100 cursor-pointer" 
                        />
                    )}
                ...
                <div className="mt-3">
                    <button 
                        onClick={() => handlers.handleFulfillRequest(item)} 
                        className="w-full bg-[#8E44AD] text-white py-2 rounded-lg text-[9px] font-black uppercase shadow-sm tap"
                    >
                        I Have This
                    </button>
