...
const CAMPUSES = [
  ...
  { id: 'UNILAG', name: 'UNILAG' }, { id: 'LASU', name: 'LASU' },
  { id: 'YABATECH', name: 'YABATECH' }, { id: 'TASUED', name: 'TASUED' },
  { id: 'OOU', name: 'OOU' }, { id: 'UNIPORT', name: 'UNIPORT' },
  { id: 'UNIDEL', name: 'UNIDEL' }, { id: 'UNIBEN', name: 'UNIBEN' },
  { id: 'UI', name: 'UI' }, { id: 'OAU', name: 'OAU' },
  { id: 'UNILORIN', name: 'UNILORIN' }, { id: 'FUTA', name: 'FUTA' },
  { id: 'LASPOTECH', name: 'LASUSTECH' }, { id: 'MAPOLY', name: 'MAPOLY' },
            ...
            <div className={`relative bg-white dark:bg-[#1a1a1a] rounded-2xl overflow-hidden shadow-sm border border-gray-100 dark:border-gray-800 transition-colors duration-300 ${item.is_admin_post ? 'ring-2 ring-yellow-400' : ''}`}>
                ...
                <div className="p-2.5">
                    ...
                    <div className="flex items-center justify-between mb-2">
                         ...
                         <span className="text-[8px] font-bold text-gray-400">{item.campus}</span>
                    </div>
    ...
    <div className="min-h-screen pb-32 transition-colors duration-300 dark:bg-black">
        ...
        <header className="sticky top-0 z-50 bg-[var(--wa-chat-bg)] dark:bg-black/90 border-b border-[var(--border)] pt-safe noselect shadow-sm backdrop-blur-md">
            <div className="px-5 py-4 flex justify-between items-center">
                ...
                <div className="flex gap-3">
                    <button onClick={toggleTheme} className="w-10 h-10 flex items-center justify-center rounded-2xl bg-[var(--surface)] dark:bg-gray-800 shadow-sm tap">{darkMode ? '☀️' : '🌙'}</button>
                    {isAdmin ? <button onClick={() => setShowAdminPanel(true)} className="bg-black dark:bg-white dark:text-black text-white px-5 py-2.5 rounded-2xl text-[10px] font-black shadow-lg tap">COMMAND</button> : <button onClick={() => setShowProModal(true)} className="bg-[var(--wa-teal)] text-white px-5 py-2.5 rounded-2xl text-[10px] font-black shadow-lg tap">PRO</button>}
                </div>
            </div>
            <div className="px-5 pb-2 flex gap-2">
             ...
             <div className="px-5 pb-3 flex gap-3 overflow-x-auto scrollbar-hide pt-2">
                {CAMPUSES.map(c => <button key={c.id} onClick={() => setActiveCampus(c.id)} className={`flex-none px-5 py-2 rounded-full text-[10px] font-black border transition tap ${activeCampus === c.id ? 'bg-[var(--wa-teal)] text-white border-transparent' : 'border-gray-200 dark:border-gray-700 text-gray-400'}`}>{c.name}</button>)}
            </div>
            <div className="px-5 py-3"><div className="search-container dark:bg-gray-800"><span className="opacity-20 text-sm mr-3">🔍</span><input className="flex-1 bg-transparent py-3 text-[13px] font-semibold outline-none wa-input dark:text-white" placeholder={viewMode === 'market' ? "Search items..." : "Search requests..."} onChange={(e) => setSearchTerm(e.target.value)} /></div></div>
        </header>
        ...
        <main id="feed" className="grid grid-cols-2 gap-3 px-4 pb-20">
            {loading ? <p className="col-span-2 text-center py-10 opacity-40">Loading...</p> : 
             ...
             displayItems.map(item => <ProductCard key={item.id} item={item} viewMode={viewMode} isAdmin={isAdmin} handlers={handlers} />)}
        </main>
                ...
                <div className="glass-3d w-full p-6 rounded-t-[32px] animate-slide-up max-h-[85vh] overflow-y-auto dark:bg-gray-900">
                    ...
                    <form onSubmit={handlePost} className="space-y-4">
                        <div className="grid grid-cols-2 gap-3">
                             <select className="wa-input dark:bg-gray-800 dark:text-white" value={form.campus} onChange={e => setForm({...form, campus: e.target.value})}>{CAMPUSES.filter(c => c.id !== 'All').map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
                             {postType === 'sell' && <select className="wa-input dark:bg-gray-800 dark:text-white" value={form.type} onChange={e => setForm({...form, type: e.target.value})}><option value="Physical">📦 Physical</option><option value="Digital">⚡ Digital</option></select>}
                        </div>
                        ...
                        <div className="border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-2xl p-6 text-center relative tap">
                            ...
                            {previewUrls.length > 0 ? <div className="flex gap-2 justify-center overflow-x-auto">{previewUrls.map((url, i) => <img key={i} src={url} className="h-16 w-16 rounded-lg object-cover shadow-sm" alt="Preview" />)}</div> : <p className="text-[9px] font-black text-gray-400 uppercase">{postType === 'sell' ? "Tap to Snap (Max 3)" : "Optional Reference Photo"}</p>}
                        </div>
                        ...
                        <button disabled={submitting} className={`w-full text-white py-4 rounded-2xl font-black shadow-xl text-lg uppercase tracking-widest tap ${postType === 'sell' ? 'bg-[var(--wa-teal)]' : 'bg-[#8E44AD]'}`}>{uploadStatus || (postType === 'sell' ? "Launch Ad" : "Post Request")}</button>
        ...
        {showAdminPanel && (
            <div className="fixed inset-0 z-[200] bg-white dark:bg-black overflow-y-auto">
                <div className="p-6">
                    <div className="flex justify-between items-center mb-8"><h2 className="text-xl font-black uppercase text-[var(--wa-teal)]">Command Center</h2><button onClick={() => setShowAdminPanel(false)} className="text-xs font-bold opacity-50 bg-gray-100 px-3 py-1 rounded-lg">CLOSE</button></div>
                    <div className="space-y-8">
                        <div className="bg-blue-50 dark:bg-blue-900/20 p-5 rounded-3xl border border-blue-100 dark:border-blue-900">
                             ...
                             {systemReport ? <div className="text-[10px] font-mono text-blue-800 dark:text-blue-300"><p>STATUS: {systemReport.status}</p><p>PRODUCTS PURGED: {systemReport.products_purged}</p><p>REQUESTS PURGED: {systemReport.requests_purged}</p></div> : <button onClick={runSystemCleanup} className="bg-blue-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase w-full">RUN SYSTEM CLEANUP</button>}
                        </div>
